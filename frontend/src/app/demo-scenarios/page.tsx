...
const VOLUME_COLORS = ["#3b82f6", "#3b82f6", "#2563eb", "#3b82f6", "#3b82f6"];
    ...
    <div className="min-h-screen bg-slate-50/80 text-slate-900">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <section className="mb-10">
          <h2 className="text-sm font-medium text-slate-500 mb-4 uppercase tracking-wide">
            Select scenario
          </h2>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 mb-10 shadow-sm transition-opacity duration-300 ease-out">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-4">
            <div>
              ...
              <p className="font-medium text-slate-900">{output.topic}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide">Keyword Monitored</p>
              <p className="font-medium text-slate-900">{output.keywordMonitored}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide">Discussion Change</p>
              <p className="font-medium text-slate-900">{output.discussionChange}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide">Spike Detected</p>
              <p className={`font-medium ${output.spikeDetected ? "text-amber-700" : "text-slate-600"}`}>
          ...
          <div className="grid gap-4 sm:grid-cols-3 mb-4">
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide mb-1">Sentiment Breakdown</p>
            ...
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide mb-1">Sentiment Score</p>
          ...
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Auto-generated Summary</p>
        ...
        <section className="grid gap-8 lg:grid-cols-2 mb-10">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <BarChart data={volumeDataWithHighlight} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="volume" radius={[4, 4, 0, 0]}>
                    {volumeDataWithHighlight.map((entry, index) => (
                      <Cell
                        key={index}
                        fill={entry.isSpike ? "#2563eb" : "rgba(59, 130, 246, 0.8)"}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            <p className="text-xs text-slate-500 mt-2">
              Spike indicator highlighted in blue when selected scenario has a spike.
            </p>
          </div>
